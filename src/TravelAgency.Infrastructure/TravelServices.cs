...
public sealed class SavedTripService(TravelDbContext db, IOptions<SecurityOptions> security, ICurrentUserService currentUserService, ILogger<SavedTripService> logger) : ISavedTripService
{
    public async Task<SavedTripResponse> SaveAsync(SaveTripRequest request, Guid? userId, CancellationToken cancellationToken)
    {
        ...
        var entity = new SavedTripEntity { Id = Guid.NewGuid(), UserId = effectiveUserId, SearchId = request.SearchId, Name = request.Name, SelectedFlightProviderOfferId = request.SelectedFlightProviderOfferId, SelectedHotelProviderHotelId = request.SelectedHotelProviderHotelId, SelectedActivityIdsJson = JsonSerializer.Serialize(request.SelectedActivityIds), CreatedAtUtc = DateTime.UtcNow };
        db.SavedTrips.Add(entity);
        db.AuditLogs.Add(new AuditLogEntity { Id = Guid.NewGuid(), UserId = entity.UserId, Action = "trip_saved", ResourceType = "SavedTrip", ResourceId = entity.Id.ToString(), CreatedAtUtc = DateTime.UtcNow });
        await db.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IReadOnlyList<object>> ListAsync(Guid? userId, CancellationToken cancellationToken)
    {
        ...
        return await query.Select(x => (object)new { x.Id, x.Name, x.Status, x.CreatedAtUtc }).ToListAsync(cancellationToken);
    }
    ...
    public async Task<bool> DeleteAsync(Guid tripId, Guid? userId, CancellationToken cancellationToken)
    {
        ...
        db.AuditLogs.Add(new AuditLogEntity { Id = Guid.NewGuid(), UserId = e.UserId, Action = "saved_trip_deleted", ResourceType = "SavedTrip", ResourceId = e.Id.ToString(), CreatedAtUtc = DateTime.UtcNow });
        await db.SaveChangesAsync(cancellationToken);
        return true;
    }
