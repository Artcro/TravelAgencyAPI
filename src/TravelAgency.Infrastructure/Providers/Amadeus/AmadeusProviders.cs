...
public sealed class AmadeusLocationProvider(IHttpClientFactory factory, AmadeusAuthClient auth, IOptions<AmadeusOptions> options, TravelDbContext db) : ILocationProvider
{
    public async Task<IReadOnlyList<LocationSuggestionDto>> SearchLocationsAsync(string query, CancellationToken cancellationToken)
    {
        ...
        c.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", await auth.GetAccessTokenAsync(cancellationToken));
        ...
        return data.EnumerateArray().Select(x =>
        {
            ...
            var country = x.TryGetProperty("address", out var aEl) && aEl.TryGetProperty("countryCode", out var cEl) ? cEl.GetString() ?? "" : "";
...
public sealed class AmadeusFlightProvider(IHttpClientFactory factory, AmadeusAuthClient auth, IOptions<AmadeusOptions> options, TravelDbContext db) : IFlightProvider
{
    public async Task<IReadOnlyList<FlightOptionDto>> SearchFlightsAsync(TripSearchRequest request, CancellationToken cancellationToken)
    {
        ...
        var c = factory.CreateClient("amadeus");
        c.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", await auth.GetAccessTokenAsync(cancellationToken));
        var q = $"originLocationCode={request.Origin}&destinationLocationCode={request.Destination}&departureDate={request.DepartureDate:yyyy-MM-dd}&adults={request.Adults}&currencyCode={request.Currency}&max={request.MaxFlightResults}";
        ...
        using var doc = JsonDocument.Parse(body);
        if (!doc.RootElement.TryGetProperty("data", out var data) || data.ValueKind != JsonValueKind.Array) throw new InvalidOperationException("Amadeus flights malformed response.");
        ...
        foreach (var o in data.EnumerateArray())
        {
            ...
            var airlineCode = o.TryGetProperty("validatingAirlineCodes", out var va) && va.ValueKind == JsonValueKind.Array && va.GetArrayLength() > 0 ? va[0].GetString() ?? string.Empty : string.Empty;
            var priceObj = o.TryGetProperty("price", out var pEl) ? pEl : default;
            var totalString = priceObj.ValueKind != JsonValueKind.Undefined && priceObj.TryGetProperty("total", out var tEl) ? tEl.GetString() : null;
            _ = decimal.TryParse(totalString, out var total);
            var currency = priceObj.ValueKind != JsonValueKind.Undefined && priceObj.TryGetProperty("currency", out var cEl) ? cEl.GetString() ?? request.Currency : request.Currency;

            var duration = string.Empty;
            var stops = 0;
            if (o.TryGetProperty("itineraries", out var itineraries) && itineraries.ValueKind == JsonValueKind.Array && itineraries.GetArrayLength() > 0)
            {
                var first = itineraries[0];
                duration = first.TryGetProperty("duration", out var dEl) ? dEl.GetString() ?? string.Empty : string.Empty;
                stops = first.TryGetProperty("segments", out var segEl) && segEl.ValueKind == JsonValueKind.Array ? Math.Max(0, segEl.GetArrayLength() - 1) : 0;
            }
