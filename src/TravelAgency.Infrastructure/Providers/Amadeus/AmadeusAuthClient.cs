...
public sealed class AmadeusAuthClient(
	IHttpClientFactory factory,
	IMemoryCache cache,
	IOptions<AmadeusOptions> options,
	ILogger<AmadeusAuthClient> logger)
{
	public async Task<string> GetAccessTokenAsync(CancellationToken cancellationToken)
	{
		if (cache.TryGetValue<string>("amadeus_token", out var token) && !string.IsNullOrWhiteSpace(token))
			return token;
		...
		using var req = new HttpRequestMessage(HttpMethod.Post, $"{options.Value.BaseUrl}/v1/security/oauth2/token")
		{
			...
			{
				["grant_type"] = "client_credentials", ["client_id"] = options.Value.ClientId,
		...
		if (string.IsNullOrWhiteSpace(token))
			throw new InvalidOperationException("Amadeus auth token missing in response.");
