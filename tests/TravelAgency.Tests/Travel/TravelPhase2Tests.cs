...
public class TravelPhase2Tests
{
	...
	public void TripValidation_Fails_InvalidRequest()
	{
		...
		var errors = validator.Validate(new TripSearchRequest
			{ Origin = "RIO", Destination = "RIO", Adults = 0, MaxFlightResults = 99 });
	...
	public async Task MockHotelProvider_Returns_Results()
	{
		...
		var results =
			await provider.SearchHotelsAsync(new TripSearchRequest { Destination = "RIO", Currency = "BRL" }, default);
	...
	public async Task MockActivityProvider_Returns_Results()
	{
		...
		var results =
			await provider.SearchActivitiesAsync(new TripSearchRequest { Destination = "RIO", Currency = "BRL" },
				default);
