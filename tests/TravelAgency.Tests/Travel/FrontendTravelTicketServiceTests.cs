...
public class FrontendTravelTicketServiceTests
{
    ...
    public async Task Maps_OneWay_Flight_Into_Frontend_Shape_With_Null_Return_Fields()
    {
        var service = CreateService(new StubFlightProvider([
            new FlightOptionDto
            {
                ...
                OutboundSegments = [new TripSegmentDto { Origin = "GRU", Destination = "JFK", DepartureAt = new DateTime(2026, 5, 10, 8, 30, 0), ArrivalAt = new DateTime(2026, 5, 10, 12, 0, 0) }]
            }
        ]));
    ...
    public async Task Maps_RoundTrip_Flight_Into_Ida_And_Volta()
    {
        var service = CreateService(new StubFlightProvider([
            new FlightOptionDto
            {
                AirlineCode = "LA", AirlineName = "LATAM", TotalPrice = new MoneyDto(3500m, "BRL"),
                OutboundSegments = [new TripSegmentDto { Origin = "GRU", Destination = "JFK", DepartureAt = new DateTime(2026, 5, 10, 8, 30, 0), ArrivalAt = new DateTime(2026, 5, 10, 12, 0, 0) }],
                ReturnSegments = [new TripSegmentDto { Origin = "JFK", Destination = "GRU", DepartureAt = new DateTime(2026, 5, 20, 15, 0, 0), ArrivalAt = new DateTime(2026, 5, 20, 22, 0, 0) }]
            }
        ]));
    ...
    {
        var service = CreateService(new StubFlightProvider([ValidFlight("LA", "LATAM")]));
