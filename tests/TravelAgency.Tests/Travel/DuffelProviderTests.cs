...
public class DuffelProviderTests
{
	...
	public async Task Sends_SupplierTimeout_And_MaxConnections_And_Uses_ListOffers_Flow()
	{
		var handler = new SequenceHandler(
			_ => new HttpResponseMessage(HttpStatusCode.Created)
				{ Content = new StringContent("""{"data":{"id":"orq_123"}}""") },
			req =>
			{
				Assert.Contains("offer_request_id=orq_123", req.RequestUri!.Query);
				Assert.Contains("limit=2", req.RequestUri!.Query);
				return new HttpResponseMessage(HttpStatusCode.OK)
					{ Content = new StringContent(ListOffersResponse(3), Encoding.UTF8, "application/json") };
			});

		var provider = CreateProvider(handler,
			new DuffelOptions
			{
				BaseUrl = "https://api.duffel.com", AccessToken = "token", Version = "v2", MaxOffersToRead = 2,
				UseReturnOffers = false
			});
	...
	public async Task Timeout_With_ReturnEmptyOnTimeout_True_Returns_Empty()
	{
		...
		var provider = CreateProvider(handler,
			new DuffelOptions
				{ BaseUrl = "https://api.duffel.com", AccessToken = "token", ReturnEmptyOnTimeout = true });
	...
	public async Task Timeout_With_ReturnEmptyOnTimeout_False_Throws_Clean_Message()
	{
		...
		var provider = CreateProvider(handler,
			new DuffelOptions
			{
				BaseUrl = "https://api.duffel.com", AccessToken = "token", ReturnEmptyOnTimeout = false,
				TimeoutSeconds = 45
			});

		var ex = await Assert.ThrowsAsync<InvalidOperationException>(() =>
			provider.SearchFlightsAsync(BasicRequest(), default));
	...
	public async Task Duffel_422_Includes_Truncated_Sanitized_Body()
	{
		...
		var provider = CreateProvider(new StaticHandler(body, HttpStatusCode.UnprocessableEntity),
			new DuffelOptions { BaseUrl = "https://api.duffel.com", AccessToken = "token" });
	...
	private static TripSearchRequest BasicRequest()
	{
		return new TripSearchRequest
		{
			Origin = "JFK", Destination = "LHR", DepartureDate = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(10)),
			Adults = 1, Currency = "USD", MaxFlightResults = 5
		};
	}

	private static DuffelFlightProvider CreateProvider(HttpMessageHandler handler, DuffelOptions opts)
	{
		var db = new TravelDbContext(new DbContextOptionsBuilder<TravelDbContext>()
	...
	private sealed class StaticHandler(string body, HttpStatusCode code) : HttpMessageHandler
	{
